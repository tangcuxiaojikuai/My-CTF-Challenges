...
题目给定一个256次的不可约多项式G，用它作为模多项式，建立了一个特征为2的256次扩域。在这之后，题目将flag编码为二进制表示的多项式，然后给出这个多项式模G下的值，要求还原flag。

把这看作商环，其实题目也就是给出了flag对应多项式在如下商环中的值：
$$
Ring = \frac{Z_2[x]}{G}
$$
把环中flag的值记为f，最直白的想法是，flag显然可以写成如下形式：
$$
flag = f + kG
$$
所以可能可以爆破下k。然而，k是多项式的形式，并且flag长为45，其对应多项式次数接近45x8=360，所以爆破不了。

突破点在于商环是可以用矩阵表示的，所以其实原问题可以转化为矩阵方程求解。而由于flag已知前后缀，并且每个字符都是可见字符，MSB均为0，所以可以将解空间固定很多维，然后再在一个小范围内对解空间进行爆破即可。
